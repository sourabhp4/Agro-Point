...
export async function POST(request) {
    try {
        ...
        const { userId, title, category, description, releaseDate, image, tags, details } = reqBody

        if ( title === '' || category === '' || description === '' || releaseDate === '' || image === '' || details === '' || !tags || tags.length === 0 ) {
        ...
        const newProduct = new Product({
            userId, title, category, description, releaseDate, image, tags, details
        })
